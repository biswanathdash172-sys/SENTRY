...
- **Bindusmita** — Owns **UI/UX Design** (the Figma/wireframe version of the dashboard before code is written) and **Frontend Generation** (building the real React version of the dashboard — alert stream, attack-chain view, verification panel, playbook, approval buttons), and shares **Performance Optimization** with Biswanath on the frontend side (fast rendering, no lag when alerts stream in).
